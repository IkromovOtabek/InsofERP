...
import { Link2, RefreshCw, UserPlus, Truck, Send, Check, RefreshCcwDot } from "lucide-react";
import { Button, FormError, FormSuccess } from "@/components/ui";
import type { ActionState } from "@/lib/action";
import { linkDriver, linkAllDrivers, adoptEcoDriver, approveDriver, syncAll, syncVehicles, resendPendingTrips } from "./actions";

const ICON = { link: Link2, all: RefreshCw, import: UserPlus, vehicles: Truck, trips: Send, approve: Check, sync: RefreshCcwDot } as const;
...
export const ImportDriverButton = ({ userId, fullName, phone, isActive }: { userId: string; fullName: string; phone: string; isActive: boolean }) =>
  <ActionButton action={() => adoptEcoDriver(userId, fullName, phone, isActive, !isActive)} label={isActive ? "Xodim qilib qo'shish" : "Qo'shish va tasdiqlash"} icon="import" done="Qo'shildi" />;
export const ApproveDriverButton = ({ employeeId }: { employeeId: string }) =>
  <ActionButton action={() => approveDriver(employeeId)} label="Tasdiqlash" icon="approve" done="Tasdiqlandi" />;
export const SyncAllButton = () => <ActionButton action={() => syncAll()} label="Ro'yxatlarni tenglashtirish" icon="sync" variant="primary" small={false} done="Tenglashtirildi" />;
