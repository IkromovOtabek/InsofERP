...
export default async function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await db.goodsReceipt.findUnique({ where: { id }, include: { supplier: true, warehouse: true, items: { include: { material: true } } } });
  if (!r) notFound();
  const total = r.items.reduce((s, i) => s + Number(i.qty) * Number(i.price), 0);
  return (
    <div>
      <PageHeader title={`Kirim ${r.docNo}`} subtitle={`${date(r.date)} · ${r.supplier.name} → ${r.warehouse.name}`} />
