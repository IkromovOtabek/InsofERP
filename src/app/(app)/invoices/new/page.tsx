...
export default async function NewInvoice({ searchParams }: { searchParams: Promise<{ orderId?: string }> }) {
  ...
  const opts = orders.map((o) => {
    ...
    return { id: o.id, orderNo: o.orderNo, customer: markedName(o.customer.name, o.customerId, marks), totalM3, deliveredM3, deliveredSum: deliveredM3 * price, totalSum: o.items.reduce((s, i) => s + Number(i.qtyM3) * Number(i.price), 0), unit: unitLabel(u ?? "m3") };
  });
  return (
    <div>
